...
- `<mesh>_C<N>_cage_ballstick.obj` — the same cage as ball-and-stick (spheres =
  carbons, cylinders = bonds); use this in Mol\*/surface viewers, where the
  polyhedron `_cage.obj` renders as a solid surface indistinguishable from the mesh.
  (Generated by `make_cage_ballstick.py`.)
