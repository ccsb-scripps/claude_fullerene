#!/usr/bin/env python3
"""Combined healer: attraction-driven glides (move heptamers toward pentamers)
+ greedy annihilation scan. Then dual -> FF relax -> fit mesh4a -> export."""
import numpy as np, json, time
from scipy.spatial import cKDTree
from collections import defaultdict
V=[];T=[]
for ln in open("/tmp/im_best.obj"):
    p=ln.split()
    if not p: continue
...
def attr(deg):
    ...
    return float(np.linalg.norm(V[hp][:,None,:]-V[pt][None,:,:],axis=2).min(axis=1).sum())/sp
t0=time.time(); rng=np.random.default_rng(3)
tri=Tri(T); best=[t[:] for t in tri.F]; bestnd=tri.nd()
print("start defects:",bestnd,flush=True)
while bestnd>0 and time.time()-t0<200:
    tri=Tri([t[:] for t in best]); temp=8.0
    greedy(tri)
    if tri.nd()<bestnd: bestnd=tri.nd(); best=[t[:] for t in tri.F]
    stuck=0
    while tri.nd()>0 and time.time()-t0<200:
        for _ in range(120):
            ...
            cur=1000*tri.nd()+attr(tri.deg); r=tri.flip(e)
            if not r: continue
...
print("HEALED valence %s defects=%d (%.0fs)"%({k:int((d==k).sum()) for k in range(3,9) if (d==k).sum()},bestnd,time.time()-t0),flush=True)
...
print("fit mean %.1f%% max %.1f%% aspect %s vs %s"%(dS.mean()/Rm*100,dS.max()/Rm*100,(sX/sX.max()).round(3),(sM/sM.max()).round(3)),flush=True)
